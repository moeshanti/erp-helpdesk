  ...
  const CustomLabel = ({ viewBox }: any) => {
    ...
    return (
      <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central">
        ...
        <tspan x={cx} y={cy + 10} dy="1.0em" fontSize="10" fontWeight="bold" fill={isDark ? "#94a3b8" : "#6b7280"} letterSpacing="0.05em" textTransform="uppercase">
            {secondaryText}
        </tspan>
      </text>
    );
...
export default function App() {
  ...
  // --- INITIALIZATION ---
  useEffect(() => {
    const loadedTickets = StorageService.loadTickets();
    const loadedUsers = StorageService.loadUsers();
  ...
  useEffect(() => {
    if (tickets.length > 0) StorageService.saveTickets(tickets);
  }, [tickets]);

  useEffect(() => {
    if (users.length > 0) StorageService.saveUsers(users);
  }, [users]);
  ...
  const handleSync = async () => {
      ...
      const result = await StorageService.syncWithGoogleSheets(tickets, users);
      if (result.success) {
  ...
  const handleUpdateTicket = (id: string, updates: Partial<Ticket>) => {
    setTickets(prev => prev.map(t => {
        ...
        if (updates.status && updates.status !== t.status && currentUser) {
            const systemComment: Comment = {
                id: `sys${Date.now()}`,
                userId: currentUser.id,
                text: `changed status from ${t.status} to ${updates.status}`,
                timestamp: new Date(),
                isSystem: true
            };
  ...
  const handleAddComment = (ticketId: string, text: string, attachments: Attachment[] = [], isResolution = false, customId?: string, isAnalyzing = false, uatSteps?: string) => {
    if (!currentUser) return;
  ...
  const handleCommentFeedback = (ticketId: string, commentId: string, feedback: 'positive' | 'negative') => {
      setTickets(prev => prev.map(t => {
          if (t.id !== ticketId) return t;
          return {
              ...t,
              comments: t.comments.map(c => c.id === commentId ? { ...c, aiAnalysisFeedback: feedback } : c)
          };
      }));
  };
  ...
  const TicketDetailView = () => {
      ...
      const handleResolve = async () => {
           ...
           handleAddComment(ticket.id, resolutionText, attachments, true, undefined, false, uatSteps);
           handleUpdateTicket(ticket.id, { status: TicketStatus.RESOLVED });
      ...
      const handleReopen = () => {
          ...
          handleAddComment(ticket.id, `Re-opened ticket. Reason: ${reopenReasonText}`, [], false);
          handleUpdateTicket(ticket.id, { status: TicketStatus.REOPENED });
  ...
  const SettingsView = () => (
    <div className="max-w-2xl mx-auto bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-8 animate-fade-in">
        ...
        <div className="space-y-8">
             {/* Data Sync */}
            <div>
                <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Data Sync</h3>
                <div className="bg-gray-50 dark:bg-slate-900 p-4 rounded-xl border border-gray-200 dark:border-slate-700">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Google Apps Script URL (JSON API)</label>
                    <input 
                        type="text" 
                        value={sheetUrl}
                        onChange={(e) => { setSheetUrl(e.target.value); StorageService.setGoogleSheetUrl(e.target.value); }}
                        placeholder="https://script.google.com/macros/s/.../exec"
                        className="w-full p-3 border border-gray-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800 dark:text-white mb-3"
                    />
                    <button 
                        onClick={handleSync}
                        disabled={syncStatus === 'syncing' || !sheetUrl}
                        className="flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium disabled:opacity-50 hover:bg-indigo-700 transition-colors"
                    >
                        {syncStatus === 'syncing' ? <RefreshCw className="h-4 w-4 mr-2 animate-spin"/> : <Database className="h-4 w-4 mr-2"/>}
                        {syncStatus === 'syncing' ? 'Syncing...' : 'Sync with Google Sheets'}
                    </button>
                    {syncStatus === 'success' && <p className="text-green-600 text-xs mt-2 font-medium">Sync successful!</p>}
                    {syncStatus === 'error' && <p className="text-red-600 text-xs mt-2 font-medium">Sync failed. Check URL.</p>}
                </div>
            </div>
